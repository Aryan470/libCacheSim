//
//  S3FIFOForgiveOnlyEvict: S3-FIFO with embedding-based forgiveness ONLY at eviction
//
//  Key difference from S3FIFOForgiveClean:
//  - Only forgives objects with freq=0 that are about to be evicted
//  - Does NOT increment freq when forgiving - just prevents the eviction
//
...
// ============================================================================
class S3FIFOOnlyEvictEmbeddingManager {
public:
  double lr_;
  double ctx_speed_;
  int recent_window_;
  ...
  S3FIFOOnlyEvictEmbeddingManager(double lr, double ctx_speed, int window, int min_access, uint64_t seed = 42)
...
} S3FIFOForgiveOnlyEvict_params_t;
...
static void S3FIFOForgiveOnlyEvict_free(cache_t *cache);
static bool S3FIFOForgiveOnlyEvict_get(cache_t *cache, const request_t *req);
static cache_obj_t *S3FIFOForgiveOnlyEvict_find(cache_t *cache, const request_t *req, const bool update_cache);
static cache_obj_t *S3FIFOForgiveOnlyEvict_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S3FIFOForgiveOnlyEvict_to_evict(cache_t *cache, const request_t *req);
static void S3FIFOForgiveOnlyEvict_evict(cache_t *cache, const request_t *req);
static bool S3FIFOForgiveOnlyEvict_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S3FIFOForgiveOnlyEvict_get_occupied_byte(const cache_t *cache);
static inline int64_t S3FIFOForgiveOnlyEvict_get_n_obj(const cache_t *cache);
static inline bool S3FIFOForgiveOnlyEvict_can_insert(cache_t *cache, const request_t *req);
static void S3FIFOForgiveOnlyEvict_parse_params(cache_t *cache, const char *cache_specific_params);
static void S3FIFOForgiveOnlyEvict_evict_small(cache_t *cache, const request_t *req);
static void S3FIFOForgiveOnlyEvict_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3FIFOForgiveOnlyEvict_init(const common_cache_params_t ccache_params,
  ...
  cache_t *cache = cache_struct_init("S3FIFOForgiveOnlyEvict", ccache_params, cache_specific_params);
  cache->cache_init = S3FIFOForgiveOnlyEvict_init;
  cache->cache_free = S3FIFOForgiveOnlyEvict_free;
  cache->get = S3FIFOForgiveOnlyEvict_get;
  cache->find = S3FIFOForgiveOnlyEvict_find;
  cache->insert = S3FIFOForgiveOnlyEvict_insert;
  cache->evict = S3FIFOForgiveOnlyEvict_evict;
  cache->remove = S3FIFOForgiveOnlyEvict_remove;
  cache->to_evict = S3FIFOForgiveOnlyEvict_to_evict;
  cache->get_n_obj = S3FIFOForgiveOnlyEvict_get_n_obj;
  cache->get_occupied_byte = S3FIFOForgiveOnlyEvict_get_occupied_byte;
  cache->can_insert = S3FIFOForgiveOnlyEvict_can_insert;
  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S3FIFOForgiveOnlyEvict_params_t));
  memset(cache->eviction_params, 0, sizeof(S3FIFOForgiveOnlyEvict_params_t));
  auto *params = static_cast<S3FIFOForgiveOnlyEvict_params_t *>(cache->eviction_params);
  params->req_local = new_request();
  params->hit_on_ghost = false;

  // Parse default params first, then user params
  S3FIFOForgiveOnlyEvict_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S3FIFOForgiveOnlyEvict_parse_params(cache, cache_specific_params);
  }
  ...
  params->embedding_manager = static_cast<void *>(
      new S3FIFOOnlyEvictEmbeddingManager(params->emb_lr, params->emb_ctx_speed,
                                  params->emb_window, params->emb_min_access, 42));

  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN,
           "S3FIFOForgiveOnlyEvict-lr%.2f-th%.2f", params->emb_lr, params->emb_threshold);
...
static void S3FIFOForgiveOnlyEvict_free(cache_t *cache) {
  auto *params = static_cast<S3FIFOForgiveOnlyEvict_params_t *>(cache->eviction_params);
  auto *emb = static_cast<S3FIFOOnlyEvictEmbeddingManager *>(params->embedding_manager);
  ...
  fprintf(stderr, "[S3FIFOForgiveOnlyEvict lr=%.2f ctx=%.4f th=%.2f w=%d] "
          "evict_main=%ld evict_small=%ld forgive_small=%ld(%.1f%%) forgive_main=%ld(%.1f%%)\n",
...
static void S3FIFOForgiveOnlyEvict_evict(cache_t *cache, const request_t *req) {
  ...
  if (main_fifo->get_occupied_byte(main_fifo) > main_fifo->cache_size ||
    ...
    S3FIFOForgiveOnlyEvict_evict_main(cache, req);
  } else {
    S3FIFOForgiveOnlyEvict_evict_small(cache, req);
  }
}
