#!/usr/bin/env python3
"""
Multi-trace Optuna tuning for S3FIFOForgiveEmbCache.
Optimizes average relative improvement over S3FIFO across training traces.
"""
...
def run_single_trace(args: Tuple) -> Tuple[str, float, float]:
    ...
    baseline_mr = run_cachesim(
        cachesim_path, trace_path, "s3fifo", cache_size_ratio,
        max_requests=max_requests
    )

    # Run S3FIFOForgiveEmbCache with params
    test_mr = run_cachesim(
        cachesim_path, trace_path, "s3fifoforgive-embcache", cache_size_ratio,
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if baseline_mr is not None and test_mr is not None:
                # Improvement in hit rate: (1-test_mr) - (1-baseline_mr) / (1-baseline_mr)
                # = (baseline_mr - test_mr) / (1 - baseline_mr)
                # Or relative reduction in miss ratio: (baseline_mr - test_mr) / baseline_mr
...
def main():
    parser = argparse.ArgumentParser(description="Multi-trace Optuna tuning")
    ...
    parser.add_argument("--study-name", type=str, default="s3fifo-embcache",
    ...
    print(f"=" * 70)
    print(f"Multi-trace Optuna Tuning for S3FIFOForgiveEmbCache")
    print(f"=" * 70)
